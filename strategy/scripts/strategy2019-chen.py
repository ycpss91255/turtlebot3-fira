...
class NodeHandle(object):
    def __init__(self):
        ...
        if(SIMULATEION_FLAG):
            ...
            self.sub_scaninfo = rospy.Subscriber(
                "robot1/scan", LaserScan, self.Set_ScanInfo)
            self.sub_odom = rospy.Subscriber(
                "robot1/odom", Odometry, self.Set_Odom)
        else:
            # actual topic Publisher
            self.pub_vel = rospy.Publisher('cmd_vel', Twist, queue_size=1)
            self.pub_arm = rospy.Publisher('/tb3/arm', Int32, queue_size=1)
            self.pub_shoot = rospy.Publisher(
                "tb3/shoot", msg_Empty, queue_size=1)
            # actual topic Subscriber
            self.sub_scaninfo = rospy.Subscriber(
                "scan", LaserScan, self.Set_ScanInfo)
            self.sub_odom = rospy.Subscriber("odom", Odometry, self.Set_Odom)
            self.sub_double = rospy.Subscriber(
                "tb3/strategy/double", Int32, self.Set_Double)
            self.sub_strategy = rospy.Subscriber(
                "tb3/strategy/strategy", Int32, self.Set_Strategy)
        # Share topic Publisher
        self.pub_moving = rospy.Publisher(
            "tb3/strategy/moving", Bool, queue_size=1)
        # Share topic Subscriber
        self.sub_balls = rospy.Subscriber(
            "tb3/ball", Int32MultiArray, self.Set_Balls)
        self.sub_objects = rospy.Subscriber(
            "tb3/object", Int32MultiArray, self.Set_Object)
        self.sub_objects2 = rospy.Subscriber(
            "tb3/object2", Int32MultiArray, self.Set_Object2)
        self.sub_start = rospy.Subscriber(
            "tb3/strategy/start", Int32, self.Set_Start)
...
class Strategy(NodeHandle):
    ...
    def Get_RP_Dis(self, pos):  # robot to pose distance
        dis = math.sqrt(pow(pos[0] - self._pos[0], 2.0) +
                        pow(pos[1] - self._pos[1], 2.0))  # p2p formula
        return dis
    ...
    def Avoidance_Strategy(self):  # Waiting for modification
        ...
        for i in range(len(self._scan) - 10):  # -10 debug
            ...
            if((i < 80 or i > 280) and self._scan[i] < 0.8):
                ...
                break
            # scan value less than avoidance distance
            ...
            if(self.state == 0):
                ...
                if(RPdis > self.error_dis):
                    if(self.prev_RPdis >= RPdis):
                        ...
                        z = self.z_speed_planning(RPang) if(
                            abs(RPang) > self.error_ang) else 0
                    else:
                        x = 0
                        z = 0
                        if(RPdis < 0.3):  # exceed
    ...
    def web_go_Strategy(self):  # chen
        if((len(self.web_axis) != 0 or len(self.web_ang) != 0) or self.state == 2):
            if(self.state == 0 or self.state == 2):
                RPang = self.Get_RP_Angle(
                    self.goal) - self._front if(self.state == 0) else self.goalang - self._front
    ...
    def Find_Ball_Strategy(self):
        if(self.state == 0):
            if(self.lostball == False):
                ...
                self.balldis = 999
                self.ballang = 999
            RPang = 90 + self.findang - \
                self._front if(self._front >= 0) else -90 + \
                self.findang - self._front
            RPang = Norm_Angle(RPang)
            if(abs(RPang) > self.error_ang):
            ...
            RPang = -90 + self.findang - \
                self._front if(self.state == 1) else 90 + \
                self.findang - self._front
            RPang = Norm_Angle(RPang)
            if(abs(RPang) > self.error_ang):
                z = self.find_ball_vel_z if(
    ...
    def Find_Ball_Strategy2(self):
        if(self.state == 0):
            if(self.lostball == False):
                ...
                self.ballang = 999
            RPang = self.findang - \
                self._front if(self._front >= 0) else - \
                self.findang - self._front
            RPang = Norm_Angle(RPang)
            if(abs(RPang) > self.error_ang):
                z = self.find_ball_vel_z if(
            ...
            RPang = -self.findang - \
                self._front if(self.state == 1) else self.findang - self._front
            RPang = Norm_Angle(RPang)
            if(abs(RPang) > self.error_ang):
                z = self.find_ball_vel_z if(
                    RPang > 0) else -self.find_ball_vel_z
    ...
    def Find_Ball_Strategy3(self, angle=45):  # chen
        if(self.state == 0):
            if(self.lostball == False):
                self.ballcolor = None
                self.balldis = 999
            ...
            RPang = -angle + self.goalang - \
                self._front if(self.state == 1)else angle + \
                self.goalang - self._front
            RPang = Norm_Angle(RPang)
            if(abs(RPang) > self.error_ang):
                z = self.find_ball_vel_z if(
                    RPang > 0) else -self.find_ball_vel_z
    ...
    def Catch_Ball_Strategy(self):  # chen
        self.lostball = False
        if(self.state == 0):
            if(self.ballang == 999):
                self.Robot_Stop()
    ...
    def Goal_Strategy(self):  # chen
        ...
        # =======================go to goal area===================
            ...
            RPang = Norm_Angle(
                180 - (self.Get_RP_Angle([2.7, self.goal[1]]) - self._front))
            print('RPang', RPang)
            if(abs(RPang) > self.error_ang):
                z = -self.vel_z if(RPang > 0) else self.vel_z
                self.Robot_Vel([0, z])
    ...
    def Short_Shoot_Strategy(self):  # chen
        front_goal = copy.deepcopy(self.goal)
        front_goal[0] = 2.5
    ...
    def Process(self):
        ...
        if(self._start):
            print(self._double_point, self._behavior, self.state,
                  self._color, self.balldis, self.ballang)
            self.Robot_Moving()
            switcher = {
    ...
    def delay_time(self, sleep_time=3):
        self.Robot_Stop()
        self.counter += 1
